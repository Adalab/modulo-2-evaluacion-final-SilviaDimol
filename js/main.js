...
function addFavorite(event) {
  ...
  const index = allDataFavorites.findIndex(
    (selectedSerie) => event.currentTarget.dataset.id === selectedSerie.id
  );
  console.log(index);
  if (index === -1) {
    ...
    // Si la serie ya está en favoritas, vamos a hacer que al pulsarla de nuevo en la búsqueda, se quite de favoritas.
    allDataFavorites.splice(index, 1);
    localStorage.setItem("allDataFavorites", JSON.stringify(allDataFavorites));
    event.currentTarget.classList.remove("selectedFavoriteSerie");
    paintFavoriteSeries(allDataFavorites);
  }
}
